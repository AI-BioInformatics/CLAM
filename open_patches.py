...
def process(bag_candidate_idx,args):
    ...
    csvpath = "/mnt/beegfs/work/H2020DeciderFicarra/gbontempo/patches/CLAM/normal/process_list_autogen.csv"
    args.data_slide_dir = "/mnt/beegfs/work/H2020DeciderFicarra/gbontempo/datasets/camelyon16/training/normal"
    output_path="/mnt/beegfs/work/H2020DeciderFicarra/gbontempo/patches/CLAM/patches/normal"
...
def save_patches(file_path, output_path, wsi,target_patch_size,
    ...
    for count, (batch, coords) in enumerate(loader):
        ...
        for image,cc in zip (batch,coords):
            imagepath=os.path.join(output_path,"_x_"+str(int(cc[0]))+"_y_"+str(int(cc[1]))+".png")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_h5_dir', type=str,
                        default="/mnt/beegfs/work/H2020DeciderFicarra/gbontempo/patches/CLAM/normal")
    ...
    executor.update_parameters(slurm_partition="prod",name="data_prep",slurm_time=600,mem_gb=10,slurm_array_parallelism=15)
    jobs = executor.map_array(process,candidates,parameters)


    while True:
        print([job.get_info() for job in jobs])
